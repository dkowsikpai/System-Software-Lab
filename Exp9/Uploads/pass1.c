...
void main(){
    ...
    FILE *codef, *opf, *symf, *newin, *flen;
    ...
    if((symf = fopen("symtab.txt", "w")) == NULL){ // loading the contents of SYMTAB
        printf("Error! opening file");
        exit(1);
    }

    if((newin = fopen("inter.txt", "w")) == NULL){ // loading the contents of Intermediate File
        printf("Error! opening file");
        exit(1);
    }

    if((flen = fopen("flen.txt", "w")) == NULL){ // loading the contents of Intermediate File
        printf("Error! opening file");
        exit(1);
    }
    ...
    fprintf(flen, "%d", loc_counter - start_addr);
    // Closing the files
    fclose(codef);
    fclose(opf);
    fclose(symf);
